...
class Linear(Module):
    ...
    def forward(self,x):
        """
            results of FW pass of layer, returns s = w*x + b
    ...
    def backward(self,x,dl_ds):
        """
            ...
            dl_ds = dl_dx.mul(sigma'(s)) which is given to him by the
            backward of activation functions.
            
            returns dl_dx_prev which the the derivative of the loss
            wrt to x of the previous layer
        """
        #ex : x = size mxn, dl_ds = size nxk
        #(m x n) * (n x k) = (m x k)
        #(25 x 4) * (4 * 2)  = (25 x 2)
        #x size : 4x25, x.t() size = 25x4
        #dl_ds size = 4x2 --> x.t()*dl_ds
        
        dl_dx_prev = self.w.mm(dl_ds.t())
        #print("ASDFOJASDIFJS",dl_dx_prev)
        #print("niquetamère",dl_ds.size())
        #print("X ta race",x.size())
        #print("X transpo",x.t().size())
        #print(self.dw.size())
        #print(self.db.size())
        self.dw.add_(x.t().mm(dl_ds))
        self.db.add_(dl_ds)
