...
def generate_disc_set(nb_sample, show_data=False):
    ...
    labels = (data.sub(0.5).pow(2).sum(1) <= (1/(2*math.pi))).long()

    data_test = torch.empty(nb_sample, 2).uniform_(0, 1)
    labels_test = (data_test.sub(0.5).pow(2).sum(1) <= (1/(2*math.pi))).long()
...
def users_choices():
    ...
    if(act_fun == 'relu' and criter == 'cross entropy'):
        adapt_lrd = None
        lr = 9e-4
        wd = 3e-5
    if(act_fun == 'relu' and criter == 'MSE'):
        adapt_lrd = None
        lr = 5e-4
        wd = 3e-5
