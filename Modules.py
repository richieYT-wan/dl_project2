...
class Linear(Module):
    ...
    def backward(self, dl_ds):
        print('linear bw: dl_ds', dl_ds.size())
        print('linear bw: selfcurrent', self.current.size())
        #self.dw = dl_ds.mm(self.current.view(1, -1))
        #self.dw = dl_ds.t().mm(self.current)
        self.dw = self.current.t().mm(dl_ds)
        print('linear bw: computed dw', self.dw.size())
        self.db = dl_ds
        print('linear bw: db', self.db.size())
        #return self.w.t().mm(dl_ds)
        print('linear bw: w', self.w.size())
...
class ReLu(Module):
    ...
    def backward(self, dl_dx):
        ...
        print('relu bw: dldx', dl_dx.size())
        print('relu bw: selfcurrent', self.current.size())
        print('relu bw: dl_ds', (dl_dx*(self.current>0).float()).size())
